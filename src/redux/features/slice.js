  ...
  async (userData, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        asErrorString(response?.message || "Registration failed")
      );
    } catch (err) {
      return rejectWithValue(
        asErrorString(err?.response?.data?.message || err?.message)
      );
    }
  }
);
...
export const login = createAsyncThunk(
  ...
  async (loginData, { rejectWithValue }) => {
    try {
      ...
      if (response?.success) return response;
      return rejectWithValue(
        asErrorString(response?.message || "Login failed")
      );
    } catch (err) {
      return rejectWithValue(
        asErrorString(err?.response?.data?.message || err?.message)
...
export const fetchProfile = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      if (response?.success) return response;
      return rejectWithValue(
        asErrorString(response?.message || "Fetch profile failed")
      );
    } catch (err) {
      return rejectWithValue(
        asErrorString(err?.response?.data?.message || err?.message)
...
export const verifyUserOtp = createAsyncThunk(
  ...
  async ({ tokenId, otp }, { rejectWithValue }) => {
    try {
      ...
      if (response?.success) return response;
      return rejectWithValue(
        asErrorString(response?.message || "OTP verification failed")
      );
    } catch (err) {
      return rejectWithValue(
        asErrorString(err?.response?.data?.message || err?.message)
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(login.fulfilled, (state, action) => {
        ...
        const token = extractToken(action.payload);
        if (!token) {
          state.error = asErrorString(
            action.payload?.message || "Login failed (no token)"
          );
          return;
        }
        state.user = user;
